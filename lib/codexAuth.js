...
const CALLBACK_BIND_ATTEMPTS = 10;
const CALLBACK_BIND_RETRY_US = 200 * 1000;
...
function parseTokenEndpointError(text) {
  const trimmed = `${text ?? ''}`.trim();
  if (!trimmed)
    return 'Unknown token exchange error';

  const json = parseJsonText(trimmed);
  if (json && typeof json === 'object') {
    const description = stringOrNull(json.error_description ?? null);
    if (description)
      return description;

    const message = stringOrNull(json.error?.message ?? null);
    if (message)
      return message;

    const code = stringOrNull(json.error?.code ?? json.error ?? null);
    if (code)
      return code;
  }
...
function bindCallbackPort(service) {
  let cancelAttempted = false;

  for (let attempt = 0; attempt < CALLBACK_BIND_ATTEMPTS; attempt++) {
    try {
      if (service.add_inet_port(CODEX_CALLBACK_PORT, null))
        return CODEX_CALLBACK_PORT;
    } catch (e) {
      if (!cancelAttempted) {
        cancelAttempted = true;
        sendCancelRequest(CODEX_CALLBACK_PORT);
      }
    }

    GLib.usleep(CALLBACK_BIND_RETRY_US);
  }
...
export async function exchangeCodexAuthorizationCode({
  ...
  try {
    ...
    return { ok: false, errorKind: 'network', errorMessage: `${e}` };
  }
  ...
  if (response.status < 200 || response.status >= 300)
    return {
      ...
      errorMessage: parseTokenEndpointError(response.text),
    };

  const tokens = parseTokenResponse(response.text);
  const auth = buildCodexAuthRecord(tokens ?? {});
  if (!auth)
    return { ok: false, errorKind: 'parse', errorMessage: 'OpenAI returned an unexpected token response.' };
...
export function startCodexBrowserLogin({ requestTextFn = requestText } = {}) {
  ...
  const port = bindCallbackPort(service);
  if (!port) {
  ...
  async function handleIncoming(connection) {
    ...
    if (requestUrl?.path === '/cancel') {
      writeHttpResponse(connection, '<html><body>Login cancelled. You can close this tab.</body></html>');
      closeConnection(connection);
    ...
    if (error) {
      const description = stringOrNull(requestUrl.params.error_description);
      const message = description ? `Sign-in failed: ${description}` : `Sign-in failed: ${error}`;
      writeHttpResponse(connection, `<html><body>${htmlEscape(message)} You can close this tab.</body></html>`, '403 Forbidden');
      closeConnection(connection);
      finish({
        ok: false,
        errorKind: 'auth',
    ...
    if (!exchanged.ok) {
      const message = htmlEscape(exchanged.errorMessage ?? 'Could not finish sign-in.');
      writeHttpResponse(connection, `<html><body>${message} You can close this tab.</body></html>`, '500 Internal Server Error');
      closeConnection(connection);
      finish(exchanged);
      return;
    }
